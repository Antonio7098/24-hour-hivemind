...
class RunManager:
    ...
    def __init__(self, state_dir: Path, session_id: str | None = None):
        ...
        self.session_id = (
            session_id or f"session-{int(datetime.now().timestamp() * 1000)}"
        )
    ...
    def create_run(
        self, item: ChecklistItem, run_dir: Path | None = None, max_attempts: int = 3
    ) -> AgentRun:
    ...
    def get_completed_runs(self) -> list[AgentRun]:
        ...
        return [
            run for run in self._runs.values() if run.status == AgentStatus.COMPLETED
        ]
    ...
    def complete(self) -> None:
        ...
        self._emit(
            "session:complete",
            {"sessionId": self.session_id, "summary": summary.to_dict()},
        )
        self.persist_state()
    ...
    def get_summary(self) -> SessionSummary:
        ...
        for r in runs:
            item_id = r.item_id
            r_started = r.started_at
            existing = latest_per_item.get(item_id)
            if item_id not in latest_per_item:
                latest_per_item[item_id] = r
            elif r_started is not None and (
                existing is None
                or existing.started_at is None
                or r_started > existing.started_at
            ):
                latest_per_item[item_id] = r
            elif (
                r_started is not None
                and existing is not None
                and existing.started_at is not None
            ):
                if r_started > existing.started_at:
                    latest_per_item[item_id] = r
        ...
        return SessionSummary(
            ...
            completed=len(
                [r for r in latest_runs if r.status == AgentStatus.COMPLETED]
            ),
    ...
    def get_status_display(self) -> str:
        ...
        if active:
            ...
            for run in active:
                ...
                lines.append(
                    f"  • {run.item_id}: {run.stage.value} ({duration}s) [attempt {run.attempt + 1}/{run.max_attempts}]"
                )
