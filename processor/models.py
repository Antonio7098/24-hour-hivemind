...
class ChecklistItem:
    ...
    def is_failed(self) -> bool:
        """Check if item is marked as failed (not paused)."""
        return "❌" in self.status and not self.is_paused()

    def is_paused(self) -> bool:
        """Check if item is paused with a checkpoint."""
        return "⏸️" in self.status or "⏱️" in self.status

    def is_pending(self) -> bool:
        """Check if item needs processing (includes paused items)."""
        return not self.is_completed() and (self.is_paused() or not self.is_failed())
...
class AgentRun:
    ...
    @classmethod
    def create(
        cls, item: ChecklistItem, run_dir: Path | None = None, max_attempts: int = 3
    ) -> "AgentRun":
    ...
    def set_status(self, status: AgentStatus, error: str | None = None) -> None:
        ...
        self._notify(
            "status", {"prev": prev.value, "current": status.value, "error": error}
        )
    ...
    def increment_attempt(self) -> None:
        ...
        self.attempt += 1
        self._notify(
            "retry", {"attempt": self.attempt, "max_attempts": self.max_attempts}
        )
